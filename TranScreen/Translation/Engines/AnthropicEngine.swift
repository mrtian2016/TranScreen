...
struct AnthropicEngine: TranslationEngine {
    ...
    init(config: EngineConfig) throws {
        ...
        // 不在 init 时校验 API Key（避免启动时触发 Keychain 弹窗）
    }
    ...
    func translate(texts: [String], from sourceLang: String, to targetLang: String) async throws -> [String] {
        ...
        let body: [String: Any] = [
            "model": modelID,
            "max_tokens": 2048,
            "system": "You are a professional translator. Translate the following numbered texts from \(sourceLang) to \(targetLang). Return ONLY the translated texts in the same numbered format.",
